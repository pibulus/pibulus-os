...
- `claude-chat.service` binds only to `172.17.0.1:9016` and is exposed through the authenticated Deck route at `/deck/claude/`. It uses CSRF/origin checks, one active run, short-lived Full-mode arm tokens, an allowlisted workspace set, and `NoNewPrivileges=true`, so browser-triggered Claude cannot sudo through this service. Its private API-key bridge is `/home/pibulus/.config/claude-chat.env`, generated from `/home/pibulus/.config/api_keys` and never tracked in Git.
